  ...
  return withPathLock([resolved.data], async () => {
    ...
    if (!before.ok) {
      if (before.error.code === "FILE_TOO_LARGE" && input.dry_run !== true) {
        return prependLargeExistingFile(input, config, resolved.data);
      }
      return before;
    }
...
async function prependLargeExistingFile(
  input: PrependInput,
  config: ScalpelConfig,
  path: string,
): Promise<DomainResult<PrependResult>> {
  ...
  if (before.data.isDirectory) {
    return failure("INVALID_INPUT", "Cannot prepend text content to a directory", path);
  }
  ...
  const afterSizeBytes = Buffer.byteLength(input.content, "utf8") + before.data.sizeBytes;
  const afterSha256 = await hashContentThenFile(input.content, path);
  const writeResult = await writeTextFileStreamForMutation({
    path,
    chunks: prependChunks(input.content, path),
    before: before.data,
    afterSha256,
    afterSizeBytes,
    maxReadBytes: config.maxReadBytes,
...
async function hashContentThenFile(content: string, path: string): Promise<string> {
...
async function* prependChunks(content: string, path: string): AsyncIterable<string | Buffer> {
  yield content;
  for await (const chunk of createReadStream(path)) {
